...
class Theme1FirefoxWith(object):
    def __init__(self):
    ...
    def run(self, n=0):
        ...
        if not os.path.exists('./pics-1FW'):
            os.mkdir('./pics-1FW') # set a folder for downloaded pictures

        for i in pic_list:
            n += 1
            img_src = i.find_element_by_xpath("//img[contains(@src,'https://theme1zhenniqian.files.wordpress.com/2021/07/green.jpg')]").get_attribute('src')
            img_name =  'pic' + str(n) + '.jpg'
            img_data = requests.get(url=img_src,headers=headers).content
            img_path = 'pics-1FW/' + img_name
...
if __name__ == '__main__':
    theme1firefoxwith = Theme1FirefoxWith()
    theme1firefoxwith.run()